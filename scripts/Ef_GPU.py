...
# 1. 环境配置 (针对 GPU 优化版)
# ==========================================
# GPU 版 QE 建议减少 MPI 进程，增加单进程内的线程数
os.environ['OMP_NUM_THREADS'] = '1'  # GPU 版建议设为 1，由 GPU 处理主要计算
os.environ['CUDA_VISIBLE_DEVICES'] = '0' # 指定使用第一块显卡
# 强制开启 GPU 卸载 (针对不同编译版本可能需要的参数)
os.environ['QUARK_ENABLE_CUDA'] = '1'

# 核心数控制：如果您是单卡，建议设置总并行进程为 1 或 2
# 这样能确保 GPU 显存不会因为 MPI 进程过多而溢出
MAX_MPI_CORES = 1
...
class QEManager:
    ...
    def generate_input(self, atoms, task_name, calc_dir, override_data=None):
        ...
        高度鲁棒的 QE 输入文件生成函数：
        1. 自动处理绝对路径，防止多进程冲突。
        2. 确保所有父目录和临时目录 (outdir) 预先创建。
        3. 强制执行大写 Namelist 块名，确保 ASE 兼容性。
        4. 动态配置 K 点和离子动力学参数。
        """
        from ase.io import write
        import os

        # 1. 确保计算目录 (calc_dir) 物理存在
        # ASE 的 write 函数在目标文件夹不存在时会直接报 FileNotFoundError
        if not os.path.exists(calc_dir):
            os.makedirs(calc_dir, exist_ok=True)

        # 2. 强制将路径转换为绝对路径，消除多进程切换工作目录时的歧义
        abs_calc_dir = os.path.abspath(calc_dir)
        
        # 为每个进程分配独立的 outdir，防止多个 pw.x 进程读写冲突
        # 增加 os.getpid() 是为了在同名任务重启时提供额外的隔离层
        unique_outdir = os.path.join(abs_calc_dir, f"tmp_{task_name}_{os.getpid()}")
        if not os.path.exists(unique_outdir):
            os.makedirs(unique_outdir, exist_ok=True)

        # 判断计算类型：氧分子/原子使用单点能 (scf)，超胞使用结构优化 (relax)
        is_relax = len(atoms) > 2 
    
        # 3. 构造标准的 QE 输入数据结构 (块名必须大写) [cite: 5, 7, 8]
        input_data = {
            'CONTROL': {
                'calculation': 'relax' if is_relax else 'scf',
                'restart_mode': 'from_scratch',
                'nstep': 100,
                'tprnfor': True,
                'prefix': f'calc_{task_name}',
                'pseudo_dir': os.path.abspath(self.pseudo_dir), # 伪势路径绝对化
                'outdir': unique_outdir,                        # 临时目录绝对化 [cite: 6]
        ...
        # 4. 只有在需要优化时才添加 &IONS 块，严禁出现空的 &IONS / 块
        if is_relax:
            input_data['IONS'] = {'ion_dynamics': 'bfgs'}

        # 5. 合并 override_data (确保外部传入的参数也能正确匹配大写块名)
        if override_data:
            for section, params in override_data.items():
                section_upper = section.upper()
                if section_upper in input_data:
                    input_data[section_upper].update(params)
                else:
                    input_data[section_upper] = params

        # 6. 确定输入文件路径并映射伪势
        input_file = os.path.join(abs_calc_dir, 'espresso.pwi')
        symbols = atoms.get_chemical_symbols()
        needed_pseudos = {k: v for k, v in self.pseudopotentials.items() if k in symbols}

        # 7. K点策略：小分子使用单 Gamma 点，固体超胞使用 2x2x2 提高精度
        kpts_setting = (1, 1, 1) if len(atoms) <= 2 else (2, 2, 2)
...
def run_and_monitor(cmd, output_file_path):
    ...
    process = subprocess.Popen(
        ...
        text=True, 
        bufsize=1,
        env=os.environ.copy()  # 关键修复：显式复制并传递当前环境变量
    )
    # 打开日志文件准备写入
...
def run_single_material_task(task_args):
    ...
    cmd_perf = f"mpirun -np 1 pw.x -input {inp_perf}"
    ...
    if e_perfect:
        ...
        for i, (label, indices) in enumerate(sorted_envs):
            ...
            # 4. 运行计算 
            cmd_vac = f"mpirun -np 1 pw.x -input {inp_vac}"
...
def main():
    ...
    try:
        ...
        run_and_monitor(f"mpirun -np 1 pw.x -input {inp_o2}",
        ...
        run_and_monitor(f"mpirun -np 1 pw.x -input {inp_atom}",
    ...
    # 准备任务列表
    all_materials = [
        ("10ScSZ", {"Zr": 90, "Sc": 20, "Y": 0}),
        ("9Sc1YSZ", {"Zr": 90, "Sc": 18, "Y": 2}),
        ("8Sc2YSZ", {"Zr": 90, "Sc": 16, "Y": 4}),
    ...
    # 设置并行数：RTX 6000 建议同时跑 2 个任务
    num_parallel = 1
    print(f"\n>>> [并行启动] 使用 Pool 同时启动 {num_parallel} 个材料计算任务...")

    # 1. 启动进程池
    with mp.Pool(processes=num_parallel) as pool:
        # pool.map 会返回一个嵌套列表：[[材料1结果], [材料2结果], ...]
