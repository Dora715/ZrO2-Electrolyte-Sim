...
os.environ['CUDA_VISIBLE_DEVICES'] = MANUAL_GPU_ID # 全局锁卡，防止程序乱跑
...
def run_single_material_task(task_args):
    ...
    # 手动指定的GPU ID
    gpu_id = MANUAL_GPU_ID  # 直接使用全局变量中手动指定的GPU ID

    print(f"\n[进程 {os.getpid()} | 手动指定显卡 GPU {gpu_id}] >>> 开始处理材料: {name}")
