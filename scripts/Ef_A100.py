...
# ==========================================
# --- A. 路径设置 ---
QE_PATH = "/root/qe/q-e-qe-7.5/bin/pw.x"
...
def run_single_material_task(task_args):
    ...
    cmd_perf = f"mpirun --allow-run-as-root -np 4 {QE_PATH} -nk 1 -input {inp_perf}"
    ...
    if e_perfect:
        ...
        for i, (label, indices) in enumerate(sorted_envs):
            ...
            cmd_vac = f"mpirun --allow-run-as-root -np 4 {QE_PATH} -nk 1 -input {inp_vac}"
...
def main():
    ...
    try:
        ...
        cmd_o2 = f"mpirun --allow-run-as-root -np 1 {QE_PATH} -input {inp_o2}"
        run_and_monitor(cmd_o2,os.path.join(dir_o2, 'espresso.pwo'), task_tag="O2_Ref")
        ...
        cmd_atom = f"mpirun --allow-run-as-root -np 1 {QE_PATH} -input {inp_atom}"
        run_and_monitor(cmd_atom,os.path.join(dir_atom, 'espresso.pwo'), task_tag="O_Atom")
